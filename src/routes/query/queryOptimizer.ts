...
export const findIndex = async (output: QueryResult) => {
  ...
  for (const table of filterTables) {
    ...
    for (const jobTableAccess of jobTableAccesses) {
      ...
      const { tableList, columnList, ast } = sqlParser.parse(
        jobTableAccess.Job.statement
      );
            ...
            for (const fromClause of fromClauseAst) {
              ...
              if (fromClause.join && fromClause.on) {
                ...
                if (leftPredicate) {
                  ...
                  output[OptimizationType.NonClusteredIndex].push(
                    tableName + "." + leftPredicate.column
                  );
                }
                if (rightPredicate) {
                  const tableName = tableAliases[rightPredicate.table];
                  output[OptimizationType.NonClusteredIndex].push(
                    tableName + "." + rightPredicate.column
                  );
                }
              }
            }
          ...
          if (selectStatement.orderby) {
            ...
            for (const orderNode of selectStatement.orderby) {
              ...
              if (orderPredicate) {
                ...
                output[OptimizationType.ClusteredIndex].push(
                  tableName + "." + orderPredicate.column
                );
              }
            }
          }
...
const handleFrom = async (
  fromClause: any,
  output: QueryResult,
  callback: (ast: AST, output: QueryResult) => void
) => {
